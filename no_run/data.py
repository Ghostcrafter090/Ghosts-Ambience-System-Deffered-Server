...
class grabber:
    def getBaseData(url):
        ...
        if otherApiKey:
            ...
            hourlyData = weatherModule.localJsonAPI(hourlyUrl, "hourly", "pictou")
            dailyData = weatherModule.localJsonAPI(dailyUrl, "daily", "pictou")
            climateData = weatherModule.localJsonAPI(climateUrl, "climate", "pictou")
            
            forecastData = weatherModule.combineHourly(forecastData, hourlyData)
            forecastData = weatherModule.combineDaily(forecastData, dailyData)
            forecastData = weatherModule.combineClimate(forecastData, climateData)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    snow = 0
            array = [speed, gusts, float(data['visibility']), snow, weather, modifier, pressure, temp, humidity, direction, cape, actualConditions, clouds]
            print(array)
            return array
        except:
            print(traceback.format_exc())
    ...
    def getSuperFastData(url):
        ...
        try:
            ...
            humidity = 0.0
        try:
            rainHour = float(data['rainHour'])
        except:
            rainHour = 0.0
        return [temp, humidity, rainHour, rainRate, windSpeeds, windGusts, out, pressure]
    ...
    def getPrecipData(url):
        ...
        try:
            snow = data["particleType"]["snow"] - 0.08
        except:
            snow = 0.0
        try:
            rain = data["particleType"]["rain"] - 0.08
        except:
            rain = 0.0
        try:
            hail = data["particleType"]["hail"] - 0.08
        except:
            hail = 0.0
        lightLevel = 0
    ...
    class lightning:
        ...
        def run():
            while not status.exit:
                try:
                    print("Websocket reset detected. Reconnecting in 10 seconds...")
                    print(10)
...
Pressure    (hPa)    : """ + str(superData[7]) + """hPa   
Rain Rate   (mm/h)   : """ + str(superData[2]) + """mm/h   
