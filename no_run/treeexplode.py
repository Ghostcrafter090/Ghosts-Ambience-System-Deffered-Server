    ...
    while not status.exit:
        ...
        if (dataList[0][7] <= -2) or ((dataList[0][7] <= 3) and (dataList[0][4] == "snow")):
            ...
            while (randb <= 10) and (countb < 10):
                ...
                audio.playSoundWindow("treecr1.mp3;treecr1.mp3", [50, 100, 65], 1.0, 0.0, 1, lowPass=[lowPass, lowPass * 3])
                countb = countb + 1
            treeNum = random.randint(1, 4)
            audio.playSoundWindow("treeex" + str(treeNum) + ".mp3;treeex" + str(treeNum) + ".mp3", [50, 100, 65], 1.0, 0.0, 1, lowPass=[lowPass, lowPass * 3])
        else:
            time.sleep(60)
