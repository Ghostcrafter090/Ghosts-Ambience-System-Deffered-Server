        ...
        if type == "walk":
            if random.random() < (((0.000003 * self.activity * math.fabs(self.mood))) / (4 - self.prop["type"])):
                ...
                if random.random() < 0.3:
                    ...
                    audioBuffer.register("g_footsteps_" + str(random.randint(0, 2)) + ".mp3", ghSpeaker, random.random() * 100, 1.0, 0.0, 0)
                    # audioEvent.run()
                else:
                    # audioEvent = audio.event()
                    audioBuffer.register("g_creak_" + "0" + ".mp3", ghSpeaker, random.random() * 100, 1.0, 0.0, 0)
                    # audioEvent.run()
        if self.prop["type"] >= 1:
            if type == "complete":
                if random.random() < (((0.00004 * self.activity * math.fabs(self.mood))) / (4 - self.prop["type"])):
                    ...
                    if 0 < chance < 166:
                        # audioEvent = audio.event()
                        audioBuffer.register("g_tap_" + str(random.randint(0, 7)) + ".mp3", ghSpeaker, random.random() * 100, 1.0, 0.0, 0)
                        # audioEvent.run()
                    if 166 < chance < 332:
                        # audioEvent = audio.event()
                        audioBuffer.register("g_snap_" + str(random.randint(0, 8)) + ".mp3", ghSpeaker, random.random() * 100, 1.0, 0.0, 0)
                        # audioEvent.run()
                    if 332 < chance < 498:
                        # audioEvent = audio.event()
                        audioBuffer.register("g_clap_" + str(random.randint(0, 1)) + ".mp3", ghSpeaker, random.random() * 100, 1.0, 0.0, 0)
                        # audioEvent.run()
                    if 498 < chance < 664:
                        # audioEvent = audio.event()
                        audioBuffer.register("g_knock_" + str(random.randint(0, 4)) + ".mp3", ghSpeaker, random.random() * 100, 1.0, 0.0, 0)
                        # audioEvent.run()
                    if 664 < chance < 830:
                        # audioEvent = audio.event()
                        audioBuffer.register("g_door_" + str(random.randint(0, 7)) + ".mp3", ghSpeaker, random.random() * 100, 1.0, 0.0, 0)
                        # audioEvent.run()
                    if 830 < chance:
                        fn = str(random.randint(0, 5))
                        vol = random.random() * 100
                        audioBuffer.playSoundWindow("g_window_" + fn + ".mp3;g_window_" + fn + ".mp3", [vol / 1.3, vol], 1.0, 0.0, 0)                      
