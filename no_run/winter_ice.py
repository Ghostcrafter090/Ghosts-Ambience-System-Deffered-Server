    ...
    while not status.exit:
        ...
        if w < 0:
            if globals.frozenPercentage < 100:
                ...
                globals.frozenDepth = globals.frozenDepth + ((0.00625 * (-w)) / status.vars["speed"])
                
        if w > 0:
            if globals.frozenDepth > 0:
                globals.frozenDepth = globals.frozenDepth - ((0.0125 * (w)) / status.vars["speed"])
            else:
                globals.frozenPercentage = globals.frozenPercentage - ((0.8333333333333333 * (w)) / status.vars["speed"])
