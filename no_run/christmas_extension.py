                    ...
                    if random.randrange(0, 70000) < bellsChance:
                        ...
                        if globals.playBells:
                            if not utils.checkShutTheFuckUp():
                                ...
                                audioBuffer.register('sleighbells_' + str(random.randrange(0, 5)) + ".mp3", ghSpeaker, 40, 1, 0, 0)
                                # audioEvent.run()
                            ...
                            if random.randrange(0, 70000) < musicBoxChance:
                                ...
                                if random.randrange(0, 2) == 1:
                                    if globals.playBells:
                                        if not utils.checkShutTheFuckUp():
                                            # audioEvent = audio.event()
                                            audioBuffer.register("jinglebells_mb.mp3", ghSpeaker, 5, 1, 0, 1)
                                            # audioEvent.run()
                                else:
                                    if globals.playBells:
                                        if not utils.checkShutTheFuckUp():
                                            # audioEvent = audio.event()
                                            audioBuffer.register("merrychristmas_mb.mp3", ghSpeaker, 5, 1, 0, 1)
                                            # audioEvent.run()
                ...
                try:
                    ...
                    if not ((globals.dateArray[2] > 12) and (globals.dateArray[3] == globals.dayTimes[5][3]) and (pytools.clock.dateArrayToUTC(globals.dateArray) < (pytools.clock.dateArrayToUTC(globals.dayTimes[5]) + 240))):
                        if random.randrange(0, 70000) < lateDayBellsChance:
                            if globals.playBells:
                                if not utils.checkShutTheFuckUp():
                                    audioBuffer.playSoundAll("lateday_bells.mp3", 10, 1, 0, 1)
            ...
            if globals.run and calc.isNiceDay() and (((globals.dateArray[1] == 11) and ((globals.dataArray[0][7] <= 0.5) or (globals.dataArray[0][3] != 0) or (globals.dataArray[0][4] == "snow"))) or (globals.dateArray[1] == 12)):
                try:
                    ...
                    if random.randrange(0, 70000) < lateNightChoirChance:
                        if not utils.checkShutTheFuckUp():
                            audioBuffer.playSoundAll("latenight_choir.mp3", 10, 1, 0, 1)
            ...
            if globals.run and (((globals.dateArray[1] == 11) and ((globals.dataArray[0][7] <= 0.5) or (globals.dataArray[0][3] != 0) or (globals.dataArray[0][4] == "snow"))) or (globals.dateArray[1] == 12)):
                try:
                    ...
                    if random.randrange(0, 350000) < mmcIdleChance:
                        if not utils.checkShutTheFuckUp():
                            audioBuffer.playSoundWindow("mmcidle.mp3;mmcidle.mp3", [20, 30], 1, 0, 1)
