            ...
            try:
                if random.random() < activity:
                    ...
                    if totalBirdSounds < (((((12 - ((pytools.clock.getDateTime()[3] + (pytools.clock.getDateTime()[4] / 60)) - 12)) * 2) * (1 + (5 * (tools.isHunter(bird) != 1)))) * ((100 - h.data.getHallowIndex(pytools.clock.dateArrayToUTC(pytools.clock.getDateTime()))) / 100))) * 20:
                        ...
                        #     _audio.audioBuffer.register(birds[bird]["sounds"][int(math.floor(random.random() * len(birds[bird]["sounds"])))], 6, (((activity / 15) * 10)) * random.random() * tools.isWater(bird) * tools.tempDiff(birds[bird], dataArray) * tools.isRaven(bird) * tools.isCrow(bird) * 16, (random.random() * 0.05) + 0.95, 0, 0)
                        #     # audioEvent.run()
                        # else:
                        ...
                        if random.random() < (1 / 10000):
                            ...
                            _audio.audioBuffer.registerWindow(birds[bird]["sounds"][soundIndex], [windowVolume, volume, windowVolume + ((volume - windowVolume) / 2)], speed, 0, 0)
...
def run():
    ...
    _audio.audioBuffer = audio.rapidFire(30)
